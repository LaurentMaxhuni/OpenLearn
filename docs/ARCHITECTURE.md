...
- Pending, rejected, and expired request payloads are not retained as learner content. If bounded lifecycle or idempotency records are needed, they expire 24 hours after terminal completion or expiration. Discovery and authorization caches have a maximum 24-hour lifetime and contain no bearer tokens.
...
- **Duplicates and retries:** reads may be retried. Mutations are retried only with the same owner, capability, idempotency key, and request fingerprint; a matching key replays or returns the in-progress operation, while a changed fingerprint is a conflict. Mutations without an idempotency key are rejected.
