...
- **Retries:** reads may be retried. Mutations may be retried only with the same owner, capability, idempotency key, and request fingerprint. A matching key returns the existing in-progress or terminal result while the full operation record exists; a changed fingerprint is `conflict`; a mutation without an idempotency key is rejected. A retryable failure may be retried with the same key only while its full operation record is available.
