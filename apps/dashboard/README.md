...
Use `pnpm --filter @openlearn/dashboard dev` for the deterministic local preview. The **Static preview** selector exposes accepted, partial, loading, empty, invalid, retryable, pending, recovering, completed, and conflict states. Build a hosted dashboard with `VITE_OPENLEARN_MODE=connected`; it calls the authenticated `/api` service boundary and expects the deployment ingress to provide the session cookie and route.
