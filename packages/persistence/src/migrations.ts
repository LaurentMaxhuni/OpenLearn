...
/**
 * Keep the executable migration text in the package as well as the reviewed
 * SQL file under migrations/. The SQL file is the operator-facing copy; this
 * embedded copy makes the built package self-contained.
 */
...
export const runMigrations = async (
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS openlearn_schema_migrations (
  ...
  const appliedRows = await pool.query<{ version: number }>(
    'SELECT version FROM openlearn_schema_migrations ORDER BY version',
  );
  ...
  for (const migration of MIGRATIONS) {
    ...
    try {
      ...
      await connection.query(
        'INSERT INTO openlearn_schema_migrations (version, name, applied_at) VALUES ($1, $2, $3)',
